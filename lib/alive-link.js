...
var DEFAULT_OPTIONS = {
  ...
  // {boolean} `false` skip URI_REGEXP checking in string.
  preferGET: [],
...
var REDIRECT_STATUS_CODE = [301, 302, 303, 307, 308];
  ...
  var lint = /*#__PURE__*/function () {
    var _ref2 = _asyncToGenerator(function* (_ref, maxRetryCount) {
      ...
      if (isRelative(uri)) {
        ...
        if (!base) {
          var _message = 'Unable to resolve the relative URI. Please check if the options.baseURI is correctly specified.';
      ...
      var result = isLocalURI ? yield isAliveLocalFile(newURI) : yield memorizedIsAliveURI(newURI, method, maxRetryCount);
      var {
        ok,
        redirected,
        redirectTo,
